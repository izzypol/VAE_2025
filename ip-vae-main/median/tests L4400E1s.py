...
for i in range(len(ip_data)):
    ...
    xp3 = torch.stack(xp3)
    xp3_avg = torch.median(xp3, dim=0).values
    diff.append((xp3_avg.detach().numpy() - ip_data[i].detach().numpy()).mean())
plt.figure()
plt.hist(diff, bins=50)
plt.plot(stats.median(diff), 0, 'ro', label="Median difference")
plt.legend()
...
plt.figure()
plt.plot(t, diff2_avg, 'ro', label="Median difference")
plt.legend()
plt.xlabel("$t$ (s)")
...
plt.figure()
plt.plot(t, diff4_avg, 'ro', label="Median difference")
plt.legend()
plt.xlabel("$t$ (s)")
plt.ylabel("Biais (%)")
plt.title("Biais (%) L4400E_1s")
