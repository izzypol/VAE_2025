...
columns = [f"IP[{i}]" for i in range(20)]  # retrieve the data and put it in a list
...
plt.xlabel("$t$ (s)")
plt.title("Démonstration L4400E_2s")
...
plt.title("Biais selon courant L4400E_2s")
...
plt.yticks(np.arange(-1, 1.5, 0.25).tolist())
plt.title("Biais selon Vp L4400E_2s")
plt.xlabel("Vp (V)")
plt.ylabel("Biais (V)")
plt.show()